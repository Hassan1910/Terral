...
class ProductHelper {
    ...
    public function getProductImageUrl($image_filename) {
        ...
        if (empty($image_filename)) {
            return 'api/uploads/products/placeholder.jpg';
        }
        ...
        if (strpos($image_filename, '/api/uploads/products/') === 0 || strpos($image_filename, 'api/uploads/products/') === 0) {
            // It's already a web path, just return it with leading slash removed
            return ltrim($image_filename, '/');
        }
        ...
        if (file_exists($image_path)) {
            return 'api/uploads/products/' . $image_filename;
        } else {
            ...
            foreach ($alt_paths as $alt_path) {
                if (file_exists($alt_path)) {
                    return 'api/uploads/products/' . basename($alt_path);
                }
            }
